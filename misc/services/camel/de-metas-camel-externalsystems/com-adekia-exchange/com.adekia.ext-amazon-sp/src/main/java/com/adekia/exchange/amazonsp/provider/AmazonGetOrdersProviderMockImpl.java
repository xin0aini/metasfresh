...
public class AmazonGetOrdersProviderMockImpl implements GetOrdersProvider
{
	@Override
	public List<OrderType> getOrders(Ctx ctx) throws Exception
	{
		...
		String createdAfter = null; // String | A date used for selecting orders created after (or at) a specified time. Only orders placed after the specified time are returned. Either the CreatedAfter parameter or the LastUpdatedAfter parameter is required. Both cannot be empty. The date must be in ISO 8601 format.
		...
		try
		{
			...
			if (getOrdersResponse != null && getOrdersResponse.getPayload() != null && getOrdersResponse.getPayload().getOrders() != null
					&& getOrderItemsResponse != null && getOrderItemsResponse.getPayload() != null && getOrderItemsResponse.getPayload().getOrderItems() != null)
			{
				List<OrderType> retValue = getOrdersResponse.getPayload().getOrders().stream()
						.map(order -> AmazonOrder.toOrderType(order, getOrderItemsResponse.getPayload().getOrderItems()))
